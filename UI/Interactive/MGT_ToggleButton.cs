...
namespace MGTool.UI
{
    public class MGT_ToggleButton
    {
        ...
        private int Width = 2, Height = 1, Margin;
        ...
        public void Draw()
        {
            ...
            int finalToggleWidth = (int)(finalBoxWidth / 2.5f);
            int finalToggleHeight = finalBoxHeight - 6;

            float leftX = Box.X + 3;
            float rightX = Box.X + Box.Width - finalToggleWidth - 3;

            float targetX = IsPressed ? rightX : leftX;

            if (Toggle.X == 0) Toggle.X = (int)leftX;

            float smoothX = MathHelper.Lerp(Toggle.X, targetX, 0.55f);

            Toggle = new Rectangle((int)smoothX, Box.Y + 3, finalToggleWidth, finalToggleHeight);
            ...
            MGT_Loader.GetSpriteBatch().Draw(currentToggleTex, new Rectangle(Toggle.X + Toggle.Width / 2, Toggle.Y + Toggle.Height / 2, Toggle.Width, Toggle.Height), null, ToggleColor * Alpha, Rotation, ToggleOrigin, SpriteEffects.None, 0f);
        }
    }
}